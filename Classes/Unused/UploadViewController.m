...
- (void)updateView
{
	if([ABCRegSession session].username)
	{
...
-(IBAction)webLinkButtonAction:(id)sender
{
	...
	NSArray *topLevelObjects = [[NSBundle mainBundle] loadNibNamed:@"ABCRegistrationView" owner:nil options:nil];
	if (topLevelObjects && topLevelObjects.count > 0 && [[topLevelObjects objectAtIndex:0] isKindOfClass:[ABCRegistrationView class]]) {
		
		ABCRegistrationView* abcRegistrationView = [topLevelObjects objectAtIndex:0];
		abcRegistrationView.delegate = self;
		...
		[window addSubview:abcRegistrationView];
	}
